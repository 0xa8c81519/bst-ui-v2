...
export class BootService {
	...
	getTotalQuantity(): Promise<BigNumber> {
		return this.paymentFarmingProxyContract.totalQuantity().then(tq => {
			return new BigNumber(tq.toString()).div(this.denominator);
		});
	}

	getTotalQuantityIn24Hours(): Promise<any> {
		return this.http.get('https://api.bstable.finance/api/v1/getTotalQuantityIn24Hours').toPromise().then((res: any) => {
			return ethers.utils.formatEther(res.data);
		});
	}
}
